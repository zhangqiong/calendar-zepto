...
( function( root, factory ) {
    if ( typeof define === 'function' ) {
        define( 'calendar', [ 'webpack-zepto' ], function( $ ) {
            return factory( root, $ );
        } );
    } else {
    ...
    $.extend( calendar.prototype, {
         ...
         */
        setSelectDate: function( curdate ,startEnd) {
        	var me = this;
        	if(!startEnd)startEnd='';
        	if(startEnd=='end'){
        		var start=new Date(me.startDate);
        		var end=new Date(curdate);
                var startNextMonth=new Date()
                if(end.getTime()-start.getTime()>86400000*29){
                    me.startDate=util.formatDate( new Date(end.getTime()-86400000*29));
                    // 并去除当前开始日期的选中
                    var lastStartItem = this.el.find('li.start');
                    lastStartItem.removeClass('start');
                    lastStartItem.removeClass('dl');
                }
        		if(start.getTime()>end.getTime()){
        			startEnd='start';
        			me.startDate=curdate;
        			me.endDate=null;
        		}
        	}
            var config = this.config,
                curdate = ( typeof curdate == 'string' ) ? curdate : util.formatDate( curdate ),
                dateNum = util.dateToNum( curdate ),

                lastSltItem = this.el.find( 'li.cur' ),
                lastStartItem = this.el.find('li.start'),
                lastEndItem = this.el.find('li.end'),
                curSltItem = $( this.el[ 0 ].querySelector( 'li[data-date="' + curdate + '"]' ) );
                curStartItem=$(this.el[0].querySelector( 'li[data-date="' + me.startDate + '"]' ) );
                curEndItem=$(this.el[0].querySelector( 'li[data-date="' + me.endDate + '"]' ) );

                if(startEnd=='start'){
                	lastStartItem.removeClass('start');
                	lastStartItem.removeClass('dl');
                	lastEndItem.removeClass('end');
            	    lastEndItem.removeClass('dl');
                }
            //先移除上次选中日期高亮
            if ( lastSltItem.length ) {
                ...
                if(lastSltItem.hasClass(startEnd)){
                	lastSltItem.removeClass(startEnd);
                }
            }
            ...
            if ( curSltItem.length ) {
                ...
                curSltItem.addClass( 'cur' ).addClass(startEnd);
                if(startEnd)curSltItem.addClass( 'dl' );
                curDateNameEl.text( config.selectDateName );
            }
            if(curStartItem.length){

            	curStartItem.addClass( 'start' );
            	curStartItem.addClass( 'dl' );
            }
            if(curEndItem.length){

            	curEndItem.addClass( 'end' );
            	curEndItem.addClass( 'dl' );
            }
